...
//In addition to updating the mobile, this sync adapter also sends the latest weather data a wearable
// both high and low temps and id (for showing matching image)
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements  GoogleApiClient.OnConnectionFailedListener, GoogleApiClient.ConnectionCallbacks  {
    ...
    public final static String TIMESTAMP = "com.example.android.sunshine.app.timestamp";
    ...
    public void onConnected(Bundle bundle) {
        Log.d("TAG11", "CONNECT CALLED REQUESTTTTTTTTTTTTTT");
    }
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            for (int i = 0; i < weatherArray.length(); i++) {
                ...
                if (i ==0) {
                    sendCurrentForecastToWear(high,low, weatherId);
                }
    ...
    private void sendCurrentForecastToWear(double maxTemp, double minTemp, int weatherId) {
        //sends updated weather data to wearable

        PutDataMapRequest request = PutDataMapRequest.create(WEARABLE_PATH).setUrgent();
        String low = Utility.formatTemperature(getContext(), minTemp);
        String high = Utility.formatTemperature(getContext(), maxTemp);
       DataMap dataMap = request.getDataMap();
        dataMap.putString(MAX_TEMP, high);
        dataMap.putString(MIN_TEMP, low);
        dataMap.putInt(CONDITION_ID, weatherId);
